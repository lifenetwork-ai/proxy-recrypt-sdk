...
describe("PRE", () => {
    ...
    test("should generate encrypted data correctly", async () => {
        const message = await fs.readFile("testdata/data/message.txt");
        ...
        const secondLevelCipherText = await preScheme.secondLevelEncryption(
            ...
            ]) // use fixed nonce for determinism
        );
        ...
        expect(
            Buffer.from(
                BN254CurveWrapper.G1ToBytes(
                    secondLevelCipherText.encryptedKey.first
                )
            ).toString("base64")
