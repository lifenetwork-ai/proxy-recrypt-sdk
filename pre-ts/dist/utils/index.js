...
export function secretToPubkey(secret, g, Z) {
    return {
 ...
 */
export async function loadKeyPairFromFile(filename) {
    // Read file
    let jsonData;
    try {
...
export async function loadReKeyFromFile(filename) {
    // Read file
    try {
...
function bigIntToBytes(value, target) {
    let tempValue = value;
...
export function base64BufferToBigInt(buffer) {
 ...
 */
export function fp12ToBytes(fp12Element, Fp) {
 ...
 */
export function fp12FromBytes(bytes, Fp, Fp2, Fp6, Fp12) {
    if (bytes.length !== 12 * Fp.BYTES) {
        throw new Error(`Invalid byte length: expected ${12 * Fp.BYTES}, got ${bytes.length}`);
    }
    const bytesPerElement = Fp.BYTES;
    // Parse in reverse order to match the Go implementation
    const c1b2a1 = Fp.fromBytes(bytes.slice(0, bytesPerElement));
    const c1b2a0 = Fp.fromBytes(bytes.slice(bytesPerElement, 2 * bytesPerElement));
    const c1b1a1 = Fp.fromBytes(bytes.slice(2 * bytesPerElement, 3 * bytesPerElement));
    const c1b1a0 = Fp.fromBytes(bytes.slice(3 * bytesPerElement, 4 * bytesPerElement));
    const c1b0a1 = Fp.fromBytes(bytes.slice(4 * bytesPerElement, 5 * bytesPerElement));
    const c1b0a0 = Fp.fromBytes(bytes.slice(5 * bytesPerElement, 6 * bytesPerElement));
    const c0b2a1 = Fp.fromBytes(bytes.slice(6 * bytesPerElement, 7 * bytesPerElement));
    const c0b2a0 = Fp.fromBytes(bytes.slice(7 * bytesPerElement, 8 * bytesPerElement));
    const c0b1a1 = Fp.fromBytes(bytes.slice(8 * bytesPerElement, 9 * bytesPerElement));
    const c0b1a0 = Fp.fromBytes(bytes.slice(9 * bytesPerElement, 10 * bytesPerElement));
    const c0b0a1 = Fp.fromBytes(bytes.slice(10 * bytesPerElement, 11 * bytesPerElement));
    const c0b0a0 = Fp.fromBytes(bytes.slice(11 * bytesPerElement, 12 * bytesPerElement));
