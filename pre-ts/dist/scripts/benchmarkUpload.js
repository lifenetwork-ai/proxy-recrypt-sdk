 ...
 */
const generateRandomFile = async (filename, sizeInBytes) => {
    console.log(`Generating random file: ${filename} (${sizeInBytes / (1024 * 1024)} MB)`);
    ...
    try {
        ...
        if (sizeInBytes > 10 * 1024 * 1024 &&
            ...
            console.log(`Progress: ${Math.round((1 - remainingBytes / sizeInBytes) * 100)}%`);
        }
...
const runBenchmarks = async () => {
    ...
    for (const file of testFileSizes) {
        ...
        const combinedSecret = await runBenchmark("Secret Reconstruction", async () => {
        ...
        const secondLevelResponse = await runBenchmark("Second Level Encryption", async () => {
            return client.secondLevelEncryption(keypair.secretKey, Buffer.from(fileContent), scalar);
        });
        ...
        console.log(`Original size: ${fileSize} bytes, Encrypted size: ${encryptedSize} bytes`);
    ...
    console.log("\n=== BENCHMARK RESULTS ===");
    console.log("File Size (KB) | Encrypted Size (KB) | Overhead (%) | Encryption Time (ms) | Throughput (KB/s)");
    console.log("-------------- | ------------------- | ------------ | -------------------- | -----------------");
    Object.entries(results).forEach(([fileName, result]) => {
        ...
        console.log(`${fileSizeKB.toFixed(2).padEnd(14)} | ${encryptedSizeKB
            .toFixed(2)
